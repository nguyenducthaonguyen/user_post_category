...
def test_should_return_categories_empty_when_create_create_post_without_categories(
    post_service,
):
    post_data = PostCreate(
...
def test_should_raise_400_when_get_posts_by_user_id_failed(post_service, mocker):
    ...
    mocker.patch.object(
        post_service.post_repo,
        "get_posts_by_user_id",
        side_effect=Exception("Mocked DB error"),
    )
...
def test_should_return_categories_empty_when_update_post_without_categories(
    post_service,
):
    post_data = PostUpdate(
