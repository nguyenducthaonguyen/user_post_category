...
def sample_token_usage(db_session):
    token_usage = [
        TokenUsageLog(
            id=i, token="test_token1", requested_at=datetime.now(timezone.utc)
        )
        for i in range(1, 12)
    ]
    ...
    token_usage += [
        TokenUsageLog(
            id=i + 21, token="test_token2", requested_at=datetime.now(timezone.utc)
        )
...
def test_should_return_true_when_token_is_rate_limited(
    rate_limiter_service, sample_token_usage
):
    # Assuming max_requests = 10 and period_seconds = 60
    is_limited = rate_limiter_service.is_rate_limited(
        "test_token1", max_requests=5, period_seconds=10
    )

    assert is_limited is True


def test_should_blacklist_token_when_black_list_success(
    rate_limiter_service, blacklist_token_service
):
