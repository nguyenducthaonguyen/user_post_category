...
def test_should_create_active_access_token_when_data_valid(active_access_token_service, sample_users, sample_active_access_tokens):
    ...
    token_data = ActiveAccessTokenCreate(
        user_id="user4",
...
def test_should_get_active_access_tokens_when_get_by_user_id(active_access_token_service):
...
def test_should_raise_400_when_delete_token_failed(active_access_token_service, mocker):
    ...
    mocker.patch.object(active_access_token_service.repo, 'delete_token', side_effect=Exception("Deletion failed"))
...
def test_should_return_true_when_delete_tokens_by_user_id_success(active_access_token_service):
    user_id = "user5"
    response = active_access_token_service.delete_tokens_by_user_id(user_id)
    assert response is True

def test_should_raise_404_when_delete_tokens_by_user_id_not_found(active_access_token_service):
...
def test_should_raise_400_when_delete_tokens_by_user_id_failed(active_access_token_service, mocker):
    # Mock the repository method to raise an exception
    mocker.patch.object(active_access_token_service.repo, 'delete_tokens_by_user_id', side_effect=Exception("Deletion failed"))
